...
/** Map a Supabase community_ideas row → the app-facing CommunityIdea shape. */
function fromRow(r: Record<string, unknown>): CommunityIdea {
  return {
    id: String(r.id),
    title: String(r.title ?? ""),
    desc: String(r.description ?? ""),
    category: (r.category as CommunityIdea["category"]) || "Other",
    votes: Number(r.votes ?? 0),
    author: String(r.username ?? "operator"),
    timestamp: r.created_at ? Date.parse(String(r.created_at)) : 0,
  };
}
...
export default function CommunityPage({ userProfile, onNotify }: CommunityPageProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!error && data) {
        ...
        // Which ideas has THIS user voted on (RLS returns only own rows)
        const u = await getAuthUser();
        if (u) {
          const { data: mine } = await supabase
            .from("community_votes").select("idea_id").eq("user_id", u.id);
          setVoted((mine || []).map(v => String((v as { idea_id: unknown }).idea_id)));
        } else {
          setVoted([]);
        }
      } else {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (supabaseAvail) {
        ...
        const { data, error } = await supabase.from("community_ideas").insert({
          user_id:     u.id,
          username,
          title:       title.trim(),
        ...
        let created = fromRow(data);
        // Author auto-upvotes their own idea (votes → 1)
        try {
          const { data: cnt } = await supabase.rpc("toggle_vote", { p_idea: data.id });
          if (typeof cnt === "number") created = { ...created, votes: cnt };
          setVoted(prev => [...prev, String(data.id)]);
        } catch { /* non-fatal */ }
        setIdeas(prev => [created, ...prev]);
      } else {
        const idea: CommunityIdea = {
          id: `idea-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
          title: title.trim(), desc: desc.trim(), category: cat,
          votes: 1, author: username, timestamp: Date.now(),
        };
        saveLocalIdeas([idea, ...loadLocalIdeas()]);
        const nv = [...loadVotedIds(username), idea.id];
        saveVotedIds(username, nv); setVoted(nv);
  ...
  const handleVote = async (id: string) => {
    ...
    // Optimistic update
    setVoted(hasVoted ? voted.filter(v => v !== id) : [...voted, id]);
    setIdeas(prev => prev.map(i => i.id === id ? { ...i, votes: i.votes + delta } : i));

    if (supabaseAvail) {
      try {
        const { data, error } = await supabase.rpc("toggle_vote", { p_idea: id });
        if (error) throw error;
        if (typeof data === "number") {
          setIdeas(prev => prev.map(i => i.id === id ? { ...i, votes: data } : i));
        }
      } catch {
        onNotify("Vote failed — refreshing.", "error");
        load();   // resync from server on failure
      }
    } else {
      const newVoted = hasVoted ? voted.filter(v => v !== id) : [...voted, id];
      saveVotedIds(username, newVoted);
      saveLocalIdeas(loadLocalIdeas().map(i => i.id === id ? { ...i, votes: i.votes + delta } : i));
    }
  };
      ...
      <div style={{ marginBottom: "2rem" }}>
        ...
        {!supabaseAvail && !loading && (
          <div style={{
            ...
            Ideas stored locally — run the Supabase schema + <code style={{ background: "rgba(250,204,21,0.1)", padding: "0 4px", borderRadius: "3px" }}>w4_community.sql</code>
            migration for cross-user sharing.
          </div>
        )}
      </div>
